function [kernel, interim_latent] = blind_deconv_2(y, lambda_pixel, lambda_grad, opts)
% 
...
%             s, num_scales, k1, k2, size(ys,1), size(ys,2));
  ...
  %% Useless operation
  if (s == num_scales)
    [~, ~, threshold]= threshold_pxpy_v1(ys,max(size(ks)),opts);
...
%     end
  ...
  %-----------------------------------------------------------%
  [ks, lambda_pixel, lambda_grad, interim_latent] = blind_deconv_main_2(ys, ks, lambda_pixel,...
      lambda_grad, threshold, opts,s);
   %% center the kernel
...
if stemp==2
    kernel = resizeKer(ks,1/ret,k1list(1),k2list(1));
    interim_latent=10;
end
